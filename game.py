...
class Engine(gregngine.Engine):
    ...
    def rezizeSprites(self):
        for sprite in self.world.sprites:
            self.world.sprites[sprite] = pygame.transform.scale(
                self.world.sprites[sprite].convert_alpha(), (self.newPixelScale, self.newPixelScale))
    ...
    def loadSaves(self):
        ...
        with open(self.savePath + "/data.save", 'rb') as f:
            ...
            self.player.inventory.hands = None if data['playerInv']['hands'] is None else Item(
                {'itemRepr': data['playerInv']['hands'], "engine": self})
    ...
    def loadGame(self):
        ...
        if not savesLoaded:
            ...
            sword = Item({'itemRepr': 'normal_sword', "engine": self,
                         'x': 10, 'y': 6, 'isGrounded': True})
            mace = Item({'itemRepr': 'normal_mace', "engine": self,
                        'x': 11, 'y': 6, 'isGrounded': True})
            bread = Item({'itemRepr': 'bread', "engine": self,
                         'x': 11, 'y': 7, 'isGrounded': True})
            bread2 = Item({'itemRepr': 'bread', "engine": self,
                          'x': 11, 'y': 7, 'isGrounded': True})
            bread3 = Item({'itemRepr': 'bread', "engine": self,
                          'x': 11, 'y': 7, 'isGrounded': True})
            ...
            self.entitiesManager.addEntity(Entity(
                {"name": "slim1", "entityRepr": "slim", "engine": self, 'x': 9, 'y': 10}))
        ...
        if pausePressed:
            if self.currentHUD in ['inventory', 'skillTree']:
                ...
                if not self.HUDMenuManager.huds['pauseMenu']['isOpen']:
                    # menus in pauseMenu
                    if self.currentHUD not in ['optionMenu']:
            ...
            for damagedInfo in self.damagesInfos:
                if entity.data['type'] in ['monster', 'player'] and entity != damagedInfo['entity'] and damagedInfo['attackRect'].colliderect(entity.rect):
                    ...
                    if 'particle' in damagedInfo:
                        damagedInfo['entity'].weaponEffectsParticles.particles.remove(
                            damagedInfo['particle'])
    ...
    def DrawWorld(self):
        ...
        for y_index, y in enumerate(self.world.world[coords['yStart']:coords['yEnd']]):
            for x_index, x in enumerate(y[coords['xStart']:coords['xEnd']]):
                coord = ((x_index*self.newPixelScale)-(coords['offx']*self.newPixelScale), (
                    y_index*self.newPixelScale)-(coords['offy']*self.newPixelScale))

                wall = self.world.walls[coords['yStart'] +
                                        y_index][coords['xStart']+x_index]
                if wall == " ":
                    self.window.blit(self.world.sprites[x], coord)

                else:
                    self.window.blits(
                        ((self.world.sprites[x], coord), (self.world.sprites[wall], coord)))
                    for entity in self.entitiesManager.visibleEntities:
                        if entity.data['type'] != 'item':
                            x, y = entity.x - \
                                (coords['xStart'] + coords['offx']), entity.y - \
                                (coords['yStart']+coords['offy'])
                            x, y = x * \
                                self.param['newPixelScale'], y * \
                                self.param['newPixelScale']
                            dist = math.hypot(coord[0]-x, coord[1]-y)
                            if abs(dist) < self.collisionsDistance:
                                if (y_index, x_index) not in self.collisions:
                                    entity.collisions[(
                                        y_index, x_index)] = coord

                                if self.debugMode == True:
                                    pygame.draw.rect(self.window, (255, 0, 0), Rect(
                                        coord, (self.newPixelScale, self.newPixelScale)))

        # self.particles.draw((500,500))

    def checkCollision(self):
        ents = [
            ent for ent in self.entitiesManager.visibleEntities if ent.data['type'] != 'item']
        for Entity in ents:
            walls = []
            ...
            for collisions in Entity.collisions.keys():
                rect = Rect(
                    Entity.collisions[collisions], (self.newPixelScale, self.newPixelScale))

                for point in points.keys():
                    if points[point]["value"] == 0:

                        points[point]["value"] = rect.collidepoint(
                            points[point]["pos"])
            ...
            for entity in self.entitiesManager.visibleEntities:
                if entity != Entity and entity.data['type'] != 'item':
                    for point in points.keys():
                        if points[point]["value"] == 0:
                            points[point]["value"] = entity.rect.collidepoint(
                                points[point]["pos"])
    ...
    def DrawHUDs(self):
        ...
        color = (255, 255, 255)
        pygame.draw.rect(self.window, color, (xM-crosshair['gap']-crosshair['length'],
                         yM-crosshair['width']/2, crosshair['length'], crosshair['width']))
        pygame.draw.rect(self.window, color, (
            xM+crosshair['gap'], yM-crosshair['width']/2, crosshair['length'], crosshair['width']))
        pygame.draw.rect(self.window, color, (
            xM-crosshair['width']/2, yM-crosshair['gap']-crosshair['length'], crosshair['width'], crosshair['length']))
        pygame.draw.rect(self.window, color, (
            xM-crosshair['width']/2, yM+crosshair['gap'], crosshair['width'], crosshair['length']))

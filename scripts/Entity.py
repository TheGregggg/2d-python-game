...
class Entity(gregngine.Entity):
    ...
    def draw(self, xStart, yStart, passThrough):
        ...
        if self.stats['health'] < self.stats['maxHealth']:
            self.blood.emitingRate = self.stats['health'] / \
                self.stats['maxHealth']
            xToDraw = self.x - xStart
            yToDraw = self.y - yStart
            coords = (int((xToDraw+0.5)*self.param['newPixelScale']), int(
                (yToDraw+0.5)*self.param['newPixelScale']))
    ...
    def death(self):
        ...
        if self.data['type'] == 'monster':
            for drop in self.stats['drops']:
                ...
                if drop['rate'] >= rng:
                    item = Item({'itemRepr': drop['loot'], "engine": self.engine, 'x': int(
                        self.x), 'y': int(self.y), 'isGrounded': True})
